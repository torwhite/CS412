...
find the determinant of $\Sigma_{i}$
 ...
 $|\Sigma_i| = \prod_{i=1}^d S_{i}^2$
 
$\log g_i(x) \triangleq \log P(C_i) + \frac{1}{(2\pi)^{d/2} |\prod_{i=1}^d S_{i}^2|^{1/2}} \exp\left(-\frac{1}{2} (\boldsymbol{x} - \boldsymbol{\mu_{i}})^T \boldsymbol{\Sigma_{i}}^{-1} (\boldsymbol{x} - \boldsymbol{\mu_{i}}) \right)$
...
simplifies to $\sum_{j=1}^{d} \frac{(x_j - \mu_{ij})^2}{S^2_{ij}}$

then plugging into discriminant function:

$\log g_i(x) \triangleq \log P(C_i) + \frac{1}{(2\pi)^{d/2} |\prod_{i=1}^d S_{i}^2|^{1/2}} \exp-{\frac{1}{2}\sum_{j=1}^{d}\frac{(x_j-\mu_{ij})^2}{S_{ij}^2}}$
